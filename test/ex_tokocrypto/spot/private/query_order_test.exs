defmodule ExTokocrypto.Spot.Private.QueryOrderTest do
  use ExUnit.Case
  ...
  describe ".query_order" do
    test "can query an order from client order id" do
      new_order_req = %ExTokocrypto.Spot.Private.Requests.CreateOrderRequest{
        ...
        price: 0.01,
        time_in_force: "GTC",
        timestamp: ExTokocrypto.Timestamp.now()
      }

      use_cassette "spot/private/create_order_before_querying_it" do
        assert {:ok,
                %ExTokocrypto.Spot.Private.Responses.CreateOrderResponse{} = create_order_response} =
                 ExTokocrypto.Spot.Private.create_order(new_order_req, @credentials)
      ...
      use_cassette "spot/private/query_order_from_client_order_id" do
        assert {:ok,
                %ExTokocrypto.Spot.Private.Responses.QueryOrderResponse{} = query_order_response} =
                 ExTokocrypto.Spot.Private.query_order_by_client_order_id(
                   "LTCBTC",
                   @client_order_id,
                   @credentials
                 )
    ...
    test "can query an order" do
      use_cassette "spot/private/query_order_from_order_id" do
        assert {:ok,
                %ExTokocrypto.Spot.Private.Responses.QueryOrderResponse{} = query_order_response} =
                 ExTokocrypto.Spot.Private.query_order_by_order_id(
                   "LTCBTC",
                   1512,
                   @credentials
                 )
