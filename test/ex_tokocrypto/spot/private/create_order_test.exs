defmodule ExTokocrypto.Spot.Private.CreateOrderTest do
  use ExUnit.Case
    ...
    describe ".create_order #{side}" do
      test "can create a good till cancel order" do
        ...
        use_cassette "spot/private/create_order_limit_#{@side}_good_til_cancel_success" do
          assert {:ok, %ExTokocrypto.Spot.Private.Responses.CreateOrderResponse{} = response} =
                   ExTokocrypto.Spot.Private.create_order(request, @credentials)
      ...
      test "bubbles other errors" do
        ...
        with_mock HTTPoison,
          ...
          assert ExTokocrypto.Spot.Private.create_order(request, @credentials) ==
                   {:error, :timeout}
        end
      end
    end
  ...
  defp build_request(side, time_in_force, client_order_id \\ nil) do
    %ExTokocrypto.Spot.Private.Requests.CreateOrderRequest{
