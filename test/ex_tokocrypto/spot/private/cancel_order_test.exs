defmodule ExTokocrypto.Spot.Private.CancelOrderTest do
  use ExUnit.Case
  ...
  describe ".cancel_order_by_order_id" do
    test "returns an ok tuple with the response" do
      ...
      use_cassette "spot/private/cancel_order_by_order_id_ok" do
        assert {:ok, response} =
                 ExTokocrypto.Spot.Private.cancel_order_by_order_id("LTCBTC", order_id, @credentials)

        assert %ExTokocrypto.Spot.Private.Responses.CancelOrderResponse{} = response
    ...
    test "returns an error tuple when the order id can't be found" do
      use_cassette "spot/private/cancel_order_by_order_id_error_not_found" do
        assert {:error, {:not_found, msg}} =
                 ExTokocrypto.Spot.Private.cancel_order_by_order_id("LTCBTC", "12345", @credentials)
