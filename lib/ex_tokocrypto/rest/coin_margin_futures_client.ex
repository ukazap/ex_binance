defmodule ExTokocrypto.Rest.CoinMarginFuturesClient do
  alias ExTokocrypto.Rest.HTTPClient
  ...
  def get(path, params, headers, opts) when is_map(params) and is_list(headers) do
    HTTPClient.coin_margin_futures_endpoint() |> HTTPClient.get(path, params, headers, opts)
  end

  def get(path, params, %Credentials{} = credentials, opts) when is_map(params) do
    HTTPClient.coin_margin_futures_endpoint() |> HTTPClient.get(path, params, credentials, opts)
  end

  @spec post(path, map, credentials, keyword) :: {:ok, any} | {:error, shared_errors}
  def post(path, params, %Credentials{} = credentials, opts \\ [])
      when is_map(params) do
    HTTPClient.coin_margin_futures_endpoint() |> HTTPClient.post(path, params, credentials, opts)
  end

  @spec delete(path, map, credentials, keyword) :: {:ok, any} | {:error, shared_errors}
  def delete(path, params, %Credentials{} = credentials, opts \\ [])
      when is_map(params) do
    HTTPClient.coin_margin_futures_endpoint()
    |> HTTPClient.delete(path, params, credentials, opts)
  end
  ...
  def put(path, params, %Credentials{} = credentials, opts \\ []) when is_map(params) do
    HTTPClient.coin_margin_futures_endpoint() |> HTTPClient.put(path, params, credentials, opts)
  end
end
